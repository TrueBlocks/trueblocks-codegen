...
func TestState_GetPreference(t *testing.T) {
	...
	t.Run("More tests", func(t *testing.T) {
		tests := []struct {
			...
			{"Unqualified key only in Org", "supportUrl", "https://org.example.com"},
...
func TestSetPreference(t *testing.T) {
	...
	t.Run("SetsAppPreferenceAndWrites", func(t *testing.T) {
		_ = state.SetPreference("app.recentlyUsedFiles", "demo1.tbproj,demo2.tbproj", true)

		orgPrefs, _ := GetAppPreferences()
		got := strings.Join(orgPrefs.RecentlyUsedFiles, ",")
		if got != "demo1.tbproj,demo2.tbproj" {
			t.Errorf("Expected app.recentlyUsedFiles to be 'demo1.tbproj,demo2.tbproj', got '%s'", got)
		}
	})
